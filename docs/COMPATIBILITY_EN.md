...
## 📱 Teclast P30T (P5A1)
- 🏷️ **Tested version:** `Android 14.0_EEA_V1.22_20240621_SZ`
...
- ⚠️ **Warning:** **NEVER** flash the Teclast `vendor_boot.img` or `super.img` to your PCBOX, or you will lose Wi-Fi, touch screen, or camera functionality. Only `boot.img` is compatible.
...
- ⚠️ **Warning:** A GSI only replaces the `system` partition (included in `super`). You must flash the GSI image to a dynamic partition, which requires being in `fastbootd` mode (not the classic bootloader fastboot).
