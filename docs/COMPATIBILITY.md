...
## Teclast P30T (P5A1)
- **Version testée :** `Android 14.0_EEA_V1.22_20240621_SZ`
...
- **Attention :** Ne flashez **JAMAIS** le fichier `vendor_boot.img` ou `super.img` de Teclast sur votre PCBOX, sous peine de perdre le Wi-Fi, l'écran tactile ou la caméra. Seul le `boot.img` est compatible.
...
- **Attention :** Un GSI remplace uniquement la partition `system` (incluse dans `super`). Il faut flasher l'image GSI sur une partition dynamique, ce qui requiert d'être en mode `fastbootd` (et non le fastboot classique du bootloader).
