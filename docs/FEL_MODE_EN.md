...
[🇫🇷 Lire en Français](FEL_MODE.md)

FEL mode is a low-level hardware emergency recovery mode integrated directly into the chip (BROM) of Allwinner processors. **This mode is unbrickable**. Even if the internal eMMC memory is entirely erased or corrupted, FEL mode will always work.

It allows the processor to communicate directly with a PC via USB to inject code or read/write to the internal storage.

## The Procedure (Button Sequence)
...
> **Note:** The tablet screen MUST remain completely black. If the brand logo or battery gauge appears, the procedure failed. Unplug, wait for it to turn off, and try again.
...
You will need to install WinUSB drivers using a tool like Zadig (https://zadig.akeo.ie/) to be able to use tools like `xfel`.
